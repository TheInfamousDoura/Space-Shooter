...
float mouseX, mouseY; // left and right
enum view {INTRO, MENU, INSTRUCTIONS, GAME, GAMEOVER};
enum view viewPage = INTRO; // initial value
bool keyStates[256] = {false}; // keyboard
bool direction[4] = {false};  //wasd -- ijkl
bool laser1Dir[2] = {false}; // direction of the lazer
bool laser2Dir[2] = {false};

int alienLife1 = 100; // life int
int alienLife2 = 100; // life alien 2
bool gameOver = false;
...
void introScreen()
{
	glClear(GL_COLOR_BUFFER_BIT);
		...
		glColor3f(1.0, 0.0, 0.0);
	displayRasterText(-425, 490, 0.0,"Faculty Of Engineering, Alexandria, Egypt");
		glColor3f(1.0, 1.0, 1.0);
	displayRasterText(-700, 385, 0.0,"DEPARTMENT OF COMPUTER SCIENCE AND COMMUNICATION");
	...
	displayRasterText(-125, 225, 0.0,"Space Shooter");
	  ...
	  displayRasterText(-800, -100, 0.0," STUDENT NAMES");
	  	glColor3f(1.0, 1.0, 1.0);
	displayRasterText(-800, -200, 0.0," Andrew Kamel & Hagar Abouroumia");
	displayRasterText(-800, -285, 0.0," Abdelrahman Khalil & Mayar Mohammed");
		glColor3f(1.0, 0.0, 0.0);
	displayRasterText(500, -100, 0.0,"Under the Guidance of");
		glColor3f(1.0, 1.0, 1.0);
	displayRasterText(500, -200, 0.0,"Doctor Mohamed Abd-ElHafeez");
		glColor3f(1.0, 0.0, 0.0);
...
void backButton() {
	if(mouseX <= -450 && mouseX >= -500 && mouseY >= -275 && mouseY <= -250){
			...
			if(mButtonPressed) {
				...
				mButtonPressed = false;
				glutPostRedisplay();
			}  // checking the position of the mouse press
	}
...
void instructionsScreenDisplay()
{
	...
	glColor3f(1, 0, 0);
	displayRasterText(-900 ,550 ,0.4 ,"INSTRUCTIONS"); // displaying
    glColor3f(1, 0, 0);
...
void DrawAlienBody(bool isPlayer1)
{
	if(isPlayer1)
		...
		glColor3f(1,1,0);		//BODY color // yellow
	glBegin(GL_POLYGON);
	for(int i=0;i<=8;i++)
		glVertex2fv(AlienBody[i]); //array alien body down
	glEnd();
...
void DrawSpaceshipBody(bool isPlayer1)
{
	...
	glPushMatrix();							//LIGHTS //when the spaceship moving
	glScalef(3,3,1);
	...
	glColor3fv(LightColor[(CI+0)%3]);
	glutSolidSphere(1,1000,1000); // render a solid or wireframe sphere respectively.
...
void DrawSteeringWheel()
{
	...
	glutWireSphere(1,8,8); // render a solid or wireframe sphere respectively.
	glPopMatrix();
...
void display()
{
	...
	glutSwapBuffers(); // Swap front- and back framebuffer during the VSync for smoother display.
}
...
void passiveMotionFunc(int x,int y) {
	...
	mouseY = -((float)y/(m_viewport[3]/700.0)-350.0); //converting screen resolution to ortho 2d spec
...
int main(int argc, char **argv)
{
	...
	glutInitDisplayMode(GLUT_SINGLE|GLUT_RGB); // Enable single buffered mode -- RGB color mode
    glutInitWindowPosition(0, 0);  // position of the window
    glutInitWindowSize(1200, 600);  // Size of the window
    glutCreateWindow("Space Shooter"); // Create window with the given title
    init();
	glutIdleFunc(refresh);  // Register callback handler if no other event "REFRESH"
    glutKeyboardFunc(keyPressed);  // Register callback handler for special-key event
	glutKeyboardUpFunc(keyReleased);  // Register callback handler for special-key event
	glutMouseFunc(mouseClick);
	glutPassiveMotionFunc(passiveMotionFunc);  //set the motion and passive motion callback respectively for the current window.
	glGetIntegerv(GL_VIEWPORT ,m_viewport); //return the value or values of a selected parameter
